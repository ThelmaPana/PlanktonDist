...
Median distance for null and plankton, on 10000 quantiles.

::: callout-note
For this, we need to generate an x axis right?
:::
...
df_all_dist %>% split(.$type) %>% map(summary)
# Difference in median of 1.4 mm
...
df_all_dist %>% 
  ...
  mutate(
    ...
    x_axis = seq(0, 1, by = 0.01) * dist_thr # add X axis sequence, i.e. percentiles times the max distance
    ) %>% 
  ggplot() +
  geom_path(aes(x = x_axis, y = diff)) +
  labs(x = "Distance (cm)", y = "Null quantiles - plankton quantiles (cm)") +
  theme_classic()
