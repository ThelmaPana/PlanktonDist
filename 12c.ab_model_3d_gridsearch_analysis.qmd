...
```{r load}
# Simulation results
sim_res <- read_parquet("data/12b.fine_simulation_results_gridsearch.parquet")
...
sim_sel <- sim_res_ks %>% 
  mutate(
    ...
    diff_kuiper_stat = abs(kuiper_stat - obs_kuiper_stat)
    ) %>% 
...
-   density bandwidth: `r format(sim_sel$h, scientific = FALSE)`

Ideally, continuous and dotted lines should be identical.

### Compare with observations: second method

Here, we compute Kuiper statistic between modeled distances after and observations.
...
# Loop over sets of parameters
sim_vs_obs_ks <- lapply(sim_res_gr, function(el) {
  # Exract distances
  dist_mod <- el %>% filter(when == "after") %>% pull(dist)
  ...
  # Compute Kuiper stat
  kt <- kuiper_stat(dist_mod_qt, plank_dist_sub_qt)
  ...
  res <- tibble(
    ...
    n_dist_obs = nrow(plank_dist_sub), # Number of observed distances
    n_dist = (n_dist_mod + n_dist_obs) / 2, # Averaged number of distances
    kuiper_stat = kt, # Kuiper statistic
    dist_mod_qt = list(dist_mod_qt), # quantiles for modelled distances
    dist_obs_qt = list(plank_dist_sub_qt) # quantiles for observed distances
  )
  return(res)
}) %>% 
  bind_rows() %>% 
  arrange(kuiper_stat) # so that rows are ordered by kuiper stat values


sim_vs_obs_ks %>% 
  mutate(set = paste0("d_length: ", d_length, "; h: ", h) %>% fct_inorder()) %>% # generate label for set, ordered by Kuiper stat values 
...
With this second method, chosen parameters are:

-   displacement length:  `r sim_vs_obs_ks %>% slice(1) %>% pull(d_length)` px

-   density bandwidth:  `r sim_vs_obs_ks %>% slice(1) %>% pull(h) %>% format(scientific = FALSE)`
