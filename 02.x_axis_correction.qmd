...
if (read_from_ecotaxa){
  ...
  # Save it
  save(sol_col, file = "data/02.sol_col.Rdata")
} else {
  load("data/02.sol_col.Rdata")
}
...
## Apply the single correction factor to data
...
# Compute distances in null data
dist_all_rand <- compute_all_dist(rand_points, n_cores = n_cores)

# Save it
save(dist_all_rand, rand_points, file = "data/02.x_corrected_null_data.Rdata")
write_parquet(plankton, sink = "data/02.x_corrected_plankton_clean.parquet")
save(plankton_sub, file = "data/02.x_corrected_plankton_sub.Rdata")
```
