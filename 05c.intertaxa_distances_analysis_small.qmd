...
# Assign colour and shape to these
df_inter_small_above <- df_inter_small %>% 
  filter(above)
  #mutate( # too many points to assign colour and shape
  #  colour = as.character(paletteer_d(`"khroma::discreterainbow"`, n = 28)),
  #  shape = rep(21:25, 6)[1:28]
  #)
```
...
ggplot() + 
  ...
  #scale_fill_manual(values = df_inter_small$colour) +
  #scale_colour_manual(values = df_inter_small$colour) +
  #scale_shape_manual(values = df_inter_small$shape) +
...
ggplot() +
  ...
  geom_polygon(data = rib_data, aes(x = log_n_dist, y = y), alpha = 0.1) +
  geom_point(data = df_inter_small %>% filter(above), aes(x = log_n_dist_small, y = log_test_stat), alpha = 0.5, size = 0.5) +
...
ggplot() +
  geom_boxplot(data = f_val_dist, aes(x = log_n_dist, y = log_test_stat, group = log_n_dist), colour = "gray", outlier.shape = NA) +
  geom_polygon(data = rib_data, aes(x = log_n_dist, y = y), alpha = 0.1) +
  geom_point(data = df_inter_scores %>% filter(above), aes(x = log_n_dist_small, y = log_test_stat, colour = z_score)) +
...
df_inter_scores %>% 
  filter(above) %>% 
  ggplot() + 
  geom_point(aes(x = test_stat, y = z_score)) +
  #scale_colour_manual(values = df_inter_small_above$colour) +
  #scale_fill_manual(values = df_inter_small_above$colour) +
  #scale_shape_manual(values = df_inter_small_above$shape) +
...
```{r save}
save(df_inter_scores, file = "data/05c.df_inter_scores_small.Rdata")
```
