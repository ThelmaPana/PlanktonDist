...
Work with a subsample of the data: 10,000 images.
:::
...
dist_ok <- left_join(
    plankton %>% filter(img_name %in% img_names) %>% count(img_name, name = "n_obj"),
...
qt_all <- mclapply(img_names, function(name) {
  ...
  # Convert to ppp
  # x is centroid_1, y is centroid_0
  points_ppp <- ppp(points$centroid_1, points$centroid_0, window = owin(xrange = c(1, vol$x), yrange = c(1, vol$y)))
