...
First, we need to learn the relationship between points recall and distance recall, because these values are not equivalent. For example, in an image with 10 points, we initially have 45 (10×9/2) unique distances. If we remove one point, the point recall is now 90%, but we now have 36 (9×8/2) distances, corresponding to a distance recall of 80% (36/45). By knowing the distance recall for each point recall, we can directly subsample computed distances.
...
rec_dist_pt <- lapply(recall_values, function(rec_val) {
  print(rec_val)  
...
```{r process}
# Loop over recall values
rec_dist <- lapply(1:nrow(rec_summ), function(i) {
  print(i)
  # Get row
  r <- rec_summ %>% slice(i)
...
```{r save}
save(rec_dist, file = "data/13.rec_dist.Rdata")
```
