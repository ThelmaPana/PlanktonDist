...
## Distances between all Appendicularia

Here we only consider images with several Appendicularia

```{r app}
my_taxon <- "Appendicularia"

app <- plankton %>% 
  filter(taxon == my_taxon) %>% 
  add_count(img_name) %>% 
  filter(n > 1)
app_img <- images %>% filter(img_name %in% app$img_name)

img_names <- app_img %>% pull(img_name)
my_img <- img_names[1:10]
name <- my_img[1]
