...
save(all_thr, file = "data/03a.all_thr.Rdata")

ggplot(all_thr) +
  geom_point(aes(x = dist_thres, y = kuiper_stat)) +
  geom_vline(xintercept = all_thr %>% arrange(desc(kuiper_stat)) %>% slice(1) %>% pull(dist_thres), colour = "red") +
  labs(x = "Distance threshold (cm)", y = "Kuiper statistic")
```
