...
2.  Assess the effect of the number of distances on the Kuiper test statistic.
:::
...
# Compute 10000-quantiles for the plot
dist_rand_qt <- quantile(dist_rand$dist, probs = seq(0, 1, length.out = 10000), names = FALSE)

# Plot it
ggplot() + geom_density(aes(x = dist_rand_qt)) + labs(x = "Distance (cm)")
ggplot() + stat_ecdf(aes(x = dist_rand_qt)) + labs(x = "Distance (cm)")
```
