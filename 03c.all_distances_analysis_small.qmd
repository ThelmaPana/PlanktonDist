...
dist_small <- df_all_dist %>% dplyr::select(dist) %>% filter(dist < dist_thr_cm) %>% pull(dist) # plankton
dist_rand_small <- df_all_dist %>% dplyr::select(dist_rand) %>% filter(dist_rand < dist_thr_cm) %>% pull(dist_rand) # null
