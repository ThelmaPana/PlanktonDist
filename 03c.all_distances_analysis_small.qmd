...
dist_small <- df_dist %>% dplyr::select(dist) %>% filter(dist < dist_thr_cm) %>% pull(dist) # plankton
dist_rand_small <- df_dist %>% dplyr::select(dist_rand) %>% filter(dist_rand < dist_thr_cm) %>% pull(dist_rand) # null

# Store this in a tibble
df_dist_small <- bind_rows(
...
ggplot() +
  ...
  #geom_point(data = df_all, aes(x = log_n_dist, y = log_test_stat), colour = "blue") +
  geom_point(aes(x = df_all$log_n_dist_small, y = log10(kt[1])), colour = "red") +
