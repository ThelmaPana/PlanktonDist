...
if (!file.exists("data/02a.plankton_dist.parquet")){
  ...
  dist_all <- dist_all %>% mutate(dist = dist * 51 / 10000)
  
  message("Done with computing plankton distances")
  write_parquet(dist_all, sink = "data/02a.all_distances_plankton.parquet")
}
...
if (!file.exists("data/02a.random_dist.parquet")) {
  ...
  # Convert from px to cm
  dist_all_rand <- dist_all_rand %>% mutate(dist = dist * 51 / 10000)
  
  message("Done with computing null distances")
  write_parquet(dist_all_rand, sink = "data/02a.all_distances_random.parquet")
}
...
#if (!exists("dist_all")) {dist_all <- read_parquet("data/02a.plankton_dist.parquet")}
...
#
## Load null distances
#if (!exists("dist_all_rand")) {dist_all_rand <- read_parquet("data/02a.random_dist.parquet")}
## compute quantiles
...
## Save
#if (sub_sample){
#  save(df_all, file = "data/02a.all_distances_sub.Rdata")  
#} else {
#  save(df_all, file = "data/02a.all_distances.Rdata")
#}
#
#
#
